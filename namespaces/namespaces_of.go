/* namespaces_of.go
   ...
   * If no PIDs are provided, and the "--subtree=<pid>" option is specified,
   ...
   conjunction with either CLONE_NEWUSER or CLONE_NEWPID. This means that all
   of the threads in a multithreaded process must be in the same user and PID
   namespaces. Therefore, it is not necessary to scan the /proc/PID/task/TID/ns
   directories to discover any further information about the shape of the user
   or PID namespace hierarchy.
*/
...
func parseCmdLineOptions() cmdLineOptions {
	...
	// If "--namespaces=<ns-list>" was specified, parse list of namespaces
	// to display, by tokenizing <ns-list> on comma delimiters, finding each
	// token string in 'namespaceMap', and adding corresponding key
	// (a CLONE_NEW* value) to 'opts.namespaces'.

	nsList := namespaceSymlinkNames // Default is all available namespaces

	if *namespacesPtr != "" {
		nsList = strings.Split(*namespacesPtr, ",")
	}
	...
	for _, nsName := range nsList {
		...
		if nsFlag == 0 {
			fmt.Println("Bad namespace for --namespaces option: " +
				nsName)
			showUsageAndExit(1)
		}
...
// The following map lists all known namespaces, mapping the CLONE_* constant
// name to the corresponding symlink filename in /proc/PID/ns.
//
// In order to support a new namespace type when it is added to the kernel,
// it should suffice to just add an entry in this map.
...
func getActualNamespaceSymlinkNames() []string {
	...
	for _, nsFile := range namespaceMap {
		if _, err := os.Stat("/proc/self/ns/" + nsFile); err == nil {
			symlinkNames = append(symlinkNames, nsFile)
		}
	}
	...
	for _, file := range procFiles {
		if file.Name()[0] >= '1' && file.Name()[0] <= '9' {
			for _, nsFile := range namespaces {
				nsi.addProcessNamespace(file.Name(),
					nsFile, opts, false)
			}
		}
	}
...
// addNamespaceToList() adds the namespace 'ns' to the namespaces list of
...
func (nsi *namespaceInfo) addUidGidPMaps() {
	...
	for _, ns := range nsi.nsList {
		if ns.nsType == CLONE_NEWUSER {
			...
			// Walk through the list of PIDs in the namespace until
			// we can successfully read the contents of a
			// /proc/PID/[ug]id_map file. (We try all PIDs in the
			// list because some PIDs may have terminated already.)
...
func readMap(pid int, mapName string) (bool, string) {
	...
	mapBytes, err := ioutil.ReadFile(mapPath)
	if err != nil {
...
func (nsi *namespaceInfo) displayNamespace(ns namespaceID, level int,
	...
	if ns == invisUserNS {
		...
		fmt.Print(indent+namespaceMap[nsi.nsList[ns].nsType]+" ", ns)
...
func printAllPIDsFor(pid int, opts cmdLineOptions) {
	...
	if err != nil {
		...
		fmt.Print("[can't open " + statusPath + "]")
		return
	}
